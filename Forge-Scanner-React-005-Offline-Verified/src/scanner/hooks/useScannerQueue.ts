...
import { MAX_JOB_ATTEMPTS, SCAN_TIMEOUTS, isTerminalErrorCode } from '../services/scannerConfig.js';
import {
  nextQueuedDocument,
  recoverDocuments,
  requeuePatch,
...
export function useScannerQueue() {
  ...
  const processFile = useCallback(
    async (doc: ScannerDocument, file: File, attempt: number, jobToken: ScanControls['token']) => {
      ...
      if (result.docType === 'bank_statement' && !result.companyNameGuess) {
        const match =
          rawText.match(/account holder[:\s]+([^\n]+)/i) ||
          rawText.match(/business name[:\s]+([^\n]+)/i) ||
          rawText.match(/customer name[:\s]+([^\n]+)/i);
        result.companyNameGuess = match?.[1]?.trim() || null;
      }
