# Forge Scanner React 013 — Clear Labels

Standalone React + Vite scanner module, prepared for direct use inside the CRM and later Electron packaging.

The Company block no longer carries a date of birth. "DOB" sat two rows under "DBA" — two near-identical abbreviations, and a date of birth is the owner's, not the company's — so it has moved to Contact beside the owner, and the DBA field is spelled out as **Trading name (DBA)**. Nothing else changed: the 60-document batch was re-run and compared with 012, 0 differences.

`Forge-Scanner-React-012-Company-Info` is untouched and remains available for rollback, as are 004 to 011.

| block | before | now |
|---|---|---|
| Company | Legal name, **DBA**, EIN, **DOB**, Address | Legal name, **Trading name (DBA)**, EIN, Address |
| Contact | Owner, Mobile ×3, Email | Owner, **Date of birth**, Mobile ×3, Email |
...
```bash
cat Forge-Scanner-React-013-Clear-Labels.zip.part* > Forge-Scanner-React-013-Clear-Labels.zip
unzip Forge-Scanner-React-013-Clear-Labels.zip
```
