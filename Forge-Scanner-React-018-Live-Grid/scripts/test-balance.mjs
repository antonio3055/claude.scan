...
/** First Fidelity: a genuine ~$23 OCR-level gap, not a solvable tie. */
const GENUINELY_UNPROVABLE = `
11/29/2025 Beginning Balance $11.84 Minimum Balance $17.74
49 Credit(s) This Period $76,403.68 Average Ledger Balance $5,300.67
211 Debil(s) This Period $75,266.74
12/31/2025 Ending Balance $1,125.10
`;

await test('an unproven block still reports its real deposit total', () =>
  assert.equal(solve(GENUINELY_UNPROVABLE).deposits, 76403.68));
await test('an unproven block reports its closest debit reading, not zero', () =>
  assert.equal(solve(GENUINELY_UNPROVABLE).withdrawals, 75266.74));
await test('an unproven block is never marked verified', () =>
  assert.equal(solve(GENUINELY_UNPROVABLE).verified, false));
await test('an unproven block reports how far out its closest reading is', () =>
  assert.equal(solve(GENUINELY_UNPROVABLE).difference, 23.68));

await test('reconciliation treats an unproven equation as unproven, not a mismatch', () => {
  const result = E.reconciliation.reconcile({
    opening: 11.84,
    ending: 1125.1,
    transactions: [],
    equation: solve(GENUINELY_UNPROVABLE),
    transactionsComplete: true,
  });
  assert.equal(result.reconciles, null);
  assert.equal(result.source, 'statement_balance_equation');
  assert.equal(result.reason, 'equation_not_provable');
  assert.equal(result.difference, 23.68);
});

await test('an unproven equation is not overridden by a worse transaction-row read', () => {
  // The row parse here is badly incomplete (a garbled multi-page OCR scan
  // would produce exactly this shape: most rows read, most debits missed).
  // The equation's own totals must still decide the result.
  const result = E.reconciliation.reconcile({
    opening: 11.84,
    ending: 1125.1,
    transactions: [
      { amount: 69403.68, direction: 'credit' },
      { amount: 1534.57, direction: 'debit' }
    ],
    equation: solve(GENUINELY_UNPROVABLE),
    transactionsComplete: true,
  });
  assert.equal(result.reconciles, null);
  assert.notEqual(result.difference, -66755.85, 'fell through to the unreliable row total');
});
