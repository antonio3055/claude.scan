# Forge Scanner React 012 — Company Info

Standalone React + Vite scanner module, prepared for direct use inside the CRM and later Electron packaging.

The statements now say who the company is. The name the bank prints, any DBA on the statement, and the address the bank writes to are all kept on the company record beside what the application says, and the differences are marked.

`Forge-Scanner-React-011-More-Banks` is untouched and remains available for rollback, as are 004 to 010.

## What the statements say the company is
...
```bash
cat Forge-Scanner-React-012-Company-Info.zip.part* > Forge-Scanner-React-012-Company-Info.zip
unzip Forge-Scanner-React-012-Company-Info.zip
```
