...
import { ocrCandidateDocuments } from '../src/scanner/services/queueRecovery.js';
...
/* ---------------- OCR is offered, never taken automatically ---------------- */

const ocrDoc = (fileId, extra) => ({ ...doc(fileId, 'needs_review'), ...extra });

await reporter.check('only files with no text layer are offered for OCR', async () => {
  const docs = [
    ocrDoc('scan', { needsOcr: true }),
    ocrDoc('readable', { needsOcr: false }),
    ocrDoc('plain', {})
  ];
  assert.deepEqual(ocrCandidateDocuments(docs).map((item) => item.fileId), ['scan']);
});

await reporter.check('a file already read by OCR is not offered again', async () => {
  const docs = [ocrDoc('done', { needsOcr: true, usedOcr: true })];
  assert.deepEqual(ocrCandidateDocuments(docs), []);
});

await reporter.check('nothing is offered when no file needs OCR', async () => {
  assert.deepEqual(ocrCandidateDocuments([doc('a', 'complete'), doc('b', 'needs_review')]), []);
  assert.deepEqual(ocrCandidateDocuments([]), []);
});
