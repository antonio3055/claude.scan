    ...
    async (doc: ScannerDocument, file: File, attempt: number, jobToken: ScanControls['token'], lane: number) => {
      ...
      try {
        if (validation.kind === 'pdf') {
          ...
          // In OCR mode the text layer is replaced by the OCR result, so it is
          // not read at all; every other mode needs it, including to notice a
          // scanned page that has no text layer and must fall back to OCR.
          const native = await extractPdfText(file, settingsNow.regularPages, controls, {
            readTextLayer: settingsNow.mode !== 'ocr'
          });
          openDoc = native.doc;
