...
export function buildLeads(documents: ScannerDocument[]): ScannerLead[] {
  ...
  groups.forEach((docs, id) => {
    ...
    leads.push({ id, companyName, ownerName, revenue: Number(revenue) || 0, extractionScore: extraction, status, docs, application, statements, mtdDocs, issues, companyInfo });
  });
...
function buildCompanyInfo(application: ScannerDocument | undefined, docs: ScannerDocument[]) {
  ...
  return {
    ...
    statementDbas,
    statementAddresses,
    // Against the application where there is one; between statements otherwise,
    // which is how a second account in another entity's name shows up.
    nameDiffers: legalName
      ? statementNames.some((name) => !sameName(name, legalName))
      : statementNames.length > 1,
    // With no DBA on the application there is nothing to differ from; the
    // statement's DBA is still reported, it is simply the only one there is.
    dbaDiffers: applicationDba
      ? statementDbas.some((dba) => !sameName(dba, applicationDba))
      : statementDbas.length > 1,
