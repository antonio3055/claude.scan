...
await reporter.check('"Electronic Credits" is recognised as a credit section', async () =>
  assert.equal(P.sectionOf('Electronic Credits'), 'credit'));
await reporter.check('"Electronic Debits" is recognised as a debit section', async () =>
  assert.equal(P.sectionOf('Electronic Debits'), 'debit'));
await reporter.check('rows under Electronic Debits are read as debits regardless of wording', async () => {
  const text = [
    'Electronic Debits',
    'Date Description Amount',
    '12/01/2025 Transfer to Employee Fuel Account $100.00',
    '12/01/2025 FORWARD FINANCIN FF $63.75'
  ].join('\n');
  const txns = P.parseTransactions(text, { statementYear: 2025 });
  assert.equal(txns.length, 2);
  assert.ok(txns.every((t) => t.direction === 'debit'), 'a row under Electronic Debits was not read as a debit');
});
