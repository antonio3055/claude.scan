...
await reporter.check('a holder row that ran into the column beside it still yields the name', async () => {
  // Real IMCU statement: the marketing column to the right of the address
  // block is joined onto the holder's own row when the PDF rows are rebuilt.
  const text = [
    'INDIANA - Statement of Account',
    'Members Credit Union',
    'RETURN SERVICE REQUESTED',
    'ABBASPOUR INC IMCU now offers Visa credit cards.',
    '7320 E 82ND STREET Get yours today!',
    'INDIANAPOLIS IN 46256 Visit IMCU.com to learn more.',
    'Your Account Balances as of 01/31'
  ].join('\n');
  const found = E.companyName.extractCompanyName(text, { bank: 'Indiana Members Credit Union' });
  assert.equal(found.legalName, 'ABBASPOUR INC');
  assert.equal(found.evidence, 'entity_suffix_split_row');
});

await reporter.check('a split row groups with the same company read cleanly elsewhere', async () => {
  assert.equal(E.companyName.companyKey('ABBASPOUR INC'), E.companyName.companyKey('Abbaspour Inc.'));
});

await reporter.check('a sentence that merely contains a suffix word is not a company', async () => {
  const text = [
    'Bank of Somewhere',
    'Please call us if any transaction on this statement is incorrect. LP',
    'Beginning balance 100.00'
  ].join('\n');
  assert.equal(E.companyName.extractCompanyName(text, { bank: null }).legalName, null);
});

await reporter.check('an address that runs into other text is still an address', async () => {
  const text = 'Suite 400 LLC Plaza something else\nP. O. Box 718 and more\nBeginning balance 100.00';
  const found = E.companyName.extractCompanyName(text, { bank: null });
  assert.ok(!/Suite|Box/i.test(found.legalName ?? ''), `picked an address: ${found.legalName}`);
});
...
await reporter.check('an application is judged on application wording, not statement wording', async () => {
  const text =
    'Business Funding Application\nLegal company name: Aaria Tees LLC\n' +
    'Business owner information\nSocial security no: 000-00-0000\n% ownership: 100\n' +
    'Requested financing amount: $150,000\nUse of funds: equipment\nBusiness start date: 01/2019\n'.repeat(4);
  const doc = E.pipeline.processDocument({ fileId: 'app', filename: 'USE_THIS_APP.pdf', rawText: text, usedOcr: false });
  assert.equal(doc.docType, 'application');
  assert.equal(doc.textQuality.trusted, true);
  assert.notEqual(doc.textQuality.reason, 'no_statement_signal');
  assert.equal(doc.status, 'complete');
});

await reporter.check('an application with no application wording is still marked down', async () => {
  const text = 'Merchant application\nFunding application\n'.padEnd(400, ' filler text');
  const quality = E.textQuality.assessTextQuality('nothing useful here '.repeat(30), { docType: 'application' });
  assert.equal(quality.trusted, false);
  assert.equal(quality.reason, 'no_application_signal');
  assert.ok(text.length > 0);
});

await reporter.check('a statement whose arithmetic adds up is trusted whatever wording it uses', async () => {
  // PNC sets its summary in two columns, so "beginning balance" never appears
  // as a phrase — but the equation proves the text was read correctly.
  const text =
    'Business Checking PNC Bank\nBalance Summary\n' +
    'Beginning Deposits and Checks and other Ending\n' +
    'balance other additions deductions balance\n' +
    '3,367.61 54,172.17 57,054.79 484.99\n' +
    'Deposits and Other Additions Checks and Other Deductions\n'.repeat(4);
  const doc = E.pipeline.processDocument({
    fileId: 'pnc', filename: 'a.pdf', rawText: text, usedOcr: false, pageCount: 6, scannedPageCount: 6
  });
  assert.equal(doc.textQuality.trusted, true);
  assert.equal(doc.textQuality.reason, 'statement_arithmetic_proven');
  assert.equal(doc.statementSummary.deposits, 54172.17);
  assert.equal(doc.reconciliation.reconciles, true);
  assert.equal(doc.reconciliation.source, 'statement_balance_equation');
});

await reporter.check('a part-read statement still reconciles from the summary the bank printed', async () => {
  const text =
  ...
  const doc = E.pipeline.processDocument({
    fileId: 'trunc', filename: 's.pdf', rawText: text, usedOcr: false, pageCount: 12, scannedPageCount: 3
  });
  assert.equal(doc.truncated, true);
  assert.equal(doc.reconciliation.reconciles, true);
  assert.equal(doc.reconciliation.source, 'statement_balance_equation');
  assert.ok(doc.reviewItems.some((item) => item.type === 'statement_truncated'));
});

await reporter.check('a part-read statement reports the bank total as revenue, not a part month', async () => {
  const text =
    'Account Statement\nBeginning Balance $381.64\nDeposits and Additions 22 19,441.82\n' +
    'Electronic Withdrawals 18 -18,477.08\nEnding Balance 57 $1,346.38\n' +
    'Transaction description daily balance\n01-02-2026 Deposit 10.00\n';
  const doc = E.pipeline.processDocument({
    fileId: 'rev', filename: 's.pdf', rawText: text, usedOcr: false, pageCount: 12, scannedPageCount: 3
  });
  assert.equal(doc.deposits.trueRevenue, 19441.82);
  assert.equal(doc.deposits.trueRevenueSource, 'statement_summary');
});

await reporter.check('a document with no text layer is reported as needing OCR, not read by it', async () => {
  const result = E.pipeline.processDocument({
    fileId: 'scan', filename: 'scanned.pdf', rawText: '', usedOcr: false, needsOcr: true,
    pageCount: 6, scannedPageCount: 3
  });
  assert.equal(result.needsOcr, true);
  assert.equal(result.extractionMethod, 'native_text', 'OCR must not have run');
  assert.ok(result.reviewItems.some((item) => item.type === 'needs_ocr'));
  assert.equal(result.status, 'needs_review');
});

await reporter.check('a readable document is never marked as needing OCR', async () => {
  const text = 'Account Statement\nBeginning Balance $381.64\nDeposits and Additions 22 19,441.82\n' +
    'Electronic Withdrawals 18 -18,477.08\nEnding Balance 57 $1,346.38\n';
  const result = E.pipeline.processDocument({
    fileId: 'ok', filename: 's.pdf', rawText: text, usedOcr: false, pageCount: 3, scannedPageCount: 3
  });
  assert.equal(result.needsOcr, false);
  assert.ok(!result.reviewItems.some((item) => item.type === 'needs_ocr'));
});

/* ---------------- what the statement says the company is ---------------- */

/** Bank of America sets its own address in a column beside the holder's. */
const BOA_HOLDER = [
  'P.O. Box 15284',
...
await reporter.check("the holder's address is read past the bank's own column", async () => {
  const address = E.holderAddress.extractHolderAddress(BOA_HOLDER, { name: 'DATALAB INFOTECH INC' });
  assert.equal(address.street, '1201 RICHARDSON DR STE 180');
  assert.equal(address.town, 'RICHARDSON');
  assert.equal(address.state, 'TX');
  assert.equal(address.postcode, '75080-4610');
});

await reporter.check("the bank's own town is never taken as the holder's", async () => {
  const address = E.holderAddress.extractHolderAddress(BOA_HOLDER, { name: 'DATALAB INFOTECH INC' });
  assert.ok(!/Tampa|Wilmington/i.test(address.full ?? ''), `picked the bank's address: ${address.full}`);
});

await reporter.check('marketing copy printed after the street is cut off', async () => {
  // Wells Fargo prints its opening hours in the column beside the address.
  const text = [
    'HELLIOS SOLUTIONS, LLC',
    'Time, Sun 9:00am-10:00pm Eastern Time:',
    '13978 WILD LUPINE ST',
    'We accept all relay calls, including 711',
    'PARKER CO 80134-4684'
  ].join('\n');
  const address = E.holderAddress.extractHolderAddress(text, { name: 'HELLIOS SOLUTIONS, LLC' });
  assert.equal(address.street, '13978 WILD LUPINE ST');
  assert.equal(address.full, '13978 WILD LUPINE ST, PARKER, CO 80134-4684');
});

await reporter.check('a town row carrying the bank column is still read', async () => {
  const text = [
    'G & S METAL PRODUCTS CO., INC.',
    '3330 E 79TH ST',
    'CLEVELAND OH 44127-1831 Online: wellsfargo.com',
    'Write: Wells Fargo Bank, N.A. (182)',
    'PO Box 63020',
    'San Francisco, CA 94163'
  ].join('\n');
  const address = E.holderAddress.extractHolderAddress(text, { name: 'G & S METAL PRODUCTS CO., INC.' });
  assert.equal(address.town, 'CLEVELAND');
  assert.equal(address.state, 'OH');
  assert.ok(!/San Francisco/i.test(address.full ?? ''));
});

await reporter.check('a mail-sort code in front of the house number is dropped', async () => {
  const text = ['ACME WIDGETS LLC', '019749 808 BANKS AVE', 'BRYAN, TX 77803'].join('\n');
  const address = E.holderAddress.extractHolderAddress(text, { name: 'ACME WIDGETS LLC' });
  assert.equal(address.street, '808 BANKS AVE');
});

await reporter.check('no address is invented when none is printed', async () => {
  const address = E.holderAddress.extractHolderAddress('ACME LLC\nAccount summary\nBeginning balance $1.00', {
    name: 'ACME LLC'
  });
  assert.equal(address.full, null);
  assert.equal(address.street, null);
});

await reporter.check('the same address written two ways is one address', async () => {
  assert.equal(
    E.holderAddress.sameAddress('11901 CONANN CT, AUSTIN, TX 78753-1234', '11901 Conann Ct, Austin TX 78753'),
    true
  );
  assert.equal(
    E.holderAddress.sameAddress('11901 CONANN CT, AUSTIN, TX 78753', '12 OTHER RD, AUSTIN, TX 78753'),
    false
  );
});

await reporter.check('a statement carries its own name, DBA and address into the result', async () => {
  const doc = E.pipeline.processDocument({
    fileId: 'id', filename: 'jan.pdf', rawText: BOA_HOLDER, usedOcr: false, pageCount: 1, scannedPageCount: 1
  });
  assert.equal(doc.statementIdentity.name, 'DATALAB INFOTECH INC');
  assert.equal(doc.statementIdentity.address, '1201 RICHARDSON DR STE 180, RICHARDSON, TX 75080-4610');
  assert.equal(doc.statementIdentity.town, 'RICHARDSON');
});

await reporter.check('an account code fused onto the name is dropped', async () => {
  assert.equal(E.companyName.stripNoisePrefix('ESTAFFLLC4812 ESTAFF LLC'), 'ESTAFF LLC');
});

await reporter.check('a business whose name starts with digits keeps them', async () => {
  for (const name of ['366 Metro Mart INC', '984 LLC', '4Bs Entertainment LLC', '1950 ORIGINALS LLC', '23HUNDRED VENTURES INC']) {
    assert.equal(E.companyName.stripNoisePrefix(name), name);
  }
});

await reporter.check('a code that does not repeat the name is left alone', async () => {
  // Only a code that restates the name is the account number fused onto it.
  assert.equal(E.companyName.stripNoisePrefix('ABC123 XYZ HOLDINGS LLC'), 'ABC123 XYZ HOLDINGS LLC');
});

await reporter.check('a transaction row is never taken as the holder name', async () => {
  const text = [
    'Statement of Account',
    'DEPOSITS AND OTHER CREDITS',
    '04-13-2026 ZEL FROM AL MAWA LLC 1,200.00',
    '04-14-2026 PAYMENT TO ACME HOLDINGS 300.00'
  ].join('\n');
  const found = E.companyName.extractCompanyName(text, { bank: null });
  assert.equal(found.legalName, null, `picked a transaction row: ${found.legalName}`);
});

await reporter.check('a transaction row dated with a month name is rejected too', async () => {
  // A Bank of Colorado export opens every row "Apr 14, 2026".
  const text = [
    'Earthworx Bobcat XXXXXXX713',
    'Transactions',
    'Apr 14, 2026 THORO CORP LLC 1,200.00',
    'Apr 13, 2026 PAYMENT ACME HOLDINGS 300.00'
  ].join('\n');
  const found = E.companyName.extractCompanyName(text, { bank: null });
  assert.equal(found.legalName, null, `picked a transaction row: ${found.legalName}`);
});
