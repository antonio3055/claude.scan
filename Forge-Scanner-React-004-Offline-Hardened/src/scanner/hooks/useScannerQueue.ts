...
import { MAX_JOB_ATTEMPTS, isTerminalErrorCode } from '../services/scannerConfig.js';
import {
  nextQueuedDocument,
  recoverDocuments,
  requeuePatch,
...
export function useScannerQueue() {
  ...
  const processFile = useCallback(
    async (doc: ScannerDocument, file: File, attempt: number) => {
      ...
      const controls: ScanControls = { token: cancellationRef.current, waitIfPaused };
  ...
  const runQueue = useCallback(async () => {
    ...
    try {
      await runScanQueue({
        ...
        processJob: async (next: ScannerDocument, attempt: number) => {
          ...
          await processFile(next, file, attempt);
        },
