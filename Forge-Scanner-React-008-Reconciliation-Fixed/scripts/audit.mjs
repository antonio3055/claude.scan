...
test('OCR cancellation terminates the worker', /abort: resetOcrWorker/.test(vendor));
...
test('OCR worker startup drops a dead pending worker', /if \(ocrWorkerPromise === pending\) await resetOcrWorker\(\);/.test(vendor));
...
test('a failed file never stops the queue', /onJobFailed/.test(runner) && /await runJobWithRetry\(job, handlers\)/.test(runner));
...
test('no second key implementation remains in the UI layer', !/replace\(\/\[\^a-z0-9\]\+\/g, ' '\)/.test(leadsLib));
...
test('a PDF with no text layer falls back to OCR', /OCR_FALLBACK_MIN_CHARS/.test(config) && /needsOcrFallback/.test(queue));
test('the OCR fallback does not change the regular-scan default', /regularPages: 3/.test(queue) && /settingsNow\.mode !== 'ocr'/.test(queue));
test(
...
test('regular page default is 3', /regularPages: 3/.test(queue));
test('OCR page default is 1', /ocrPages: 1/.test(queue));
