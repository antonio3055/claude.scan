...
export function useScannerQueue() {
  ...
  const processFile = useCallback(
    async (doc: ScannerDocument, file: File, attempt: number, jobToken: ScanControls['token']) => {
      ...
      const controls: ScanControls = { token: jobToken, waitIfPaused };
      ...
      try {
        if (validation.kind === 'pdf') {
          ...
          const nativeChars = native.fullText.replace(/\s+/g, '').length;
          // A PDF with no usable text layer is a scan. Reading it needs OCR
          // whatever the scan mode says, so that one file falls back.
          const needsOcrFallback = settingsNow.mode !== 'ocr' && nativeChars < OCR_FALLBACK_MIN_CHARS;

          if (settingsNow.mode === 'ocr' || needsOcrFallback) {
            await updateDoc(doc.fileId, { processingStatus: 'ocr', stageLog });
            stamp(needsOcrFallback ? 'ocr_fallback_no_text_layer' : 'ocr_started');
            const ocr = await ocrPdfPages(native.doc, settingsNow.ocrPages, controls);
            rawText = ocr.text;
            scannedPageCount = ocr.scannedPageCount;
            usedOcr = true;
            ocrFallback = needsOcrFallback;
            stamp('ocr_completed');
          } else {
            rawText = native.fullText;
          }
        } else {
          if (settingsNow.mode !== 'ocr') {
            throw scanError('Image files require OCR. Turn OCR on in Scan Settings.', 'ocr_required_for_image');
          }
          await updateDoc(doc.fileId, { processingStatus: 'ocr', stageLog });
          stamp('ocr_started');
  ...
  const runQueue = useCallback(async () => {
    ...
    try {
      await runScanQueue({
        getNext: () => nextQueuedDocument(docsRef.current),
        ...
        processJob: async (next: ScannerDocument, attempt: number, jobToken: ScanControls['token']) => {
          ...
          await processFile(next, file, attempt, jobToken);
        },

        // A failed or cancelled job leaves PDF.js and Tesseract in an unknown
        // state. Both are torn down before the retry and before the next file.
        resetWorkers: async (error: any) => {
