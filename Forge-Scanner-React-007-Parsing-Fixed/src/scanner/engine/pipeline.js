...
(function (root) {
  ...
  function processDocument(input) {
    ...
    // Only part of the statement may have been read. The summary totals on
    // page 1 cover the whole month, so comparing them against transactions from
    // a few pages would report a mismatch that says nothing about the document.
    ...
    if (truncated) {
      ...
      reconciliation.reason = 'statement_truncated';
    ...
    } else if (reconciliation.reconciles === false) {
