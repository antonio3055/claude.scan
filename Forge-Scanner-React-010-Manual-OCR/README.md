# Forge Scanner React 010 — Manual OCR

Standalone React + Vite scanner module, prepared for direct use inside the CRM and later Electron packaging.

OCR is off and never starts on its own. A regular scan reports which files need it; they are read only when **Run OCR on flagged** is clicked. The regular scan is faster for it, and the figures are unchanged: the full flow was compared against 009 field by field — 22 fields per document, 0 differences.

`Forge-Scanner-React-009-Speed-Tuned` is untouched and remains available for rollback, as are 004 to 008.
...
```bash
cat Forge-Scanner-React-010-Manual-OCR.zip.part* > Forge-Scanner-React-010-Manual-OCR.zip
unzip Forge-Scanner-React-010-Manual-OCR.zip
```
