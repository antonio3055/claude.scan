...
test('a PDF with no text layer is reported as needing OCR', /USABLE_TEXT_MIN_CHARS/.test(config) && /needsOcr = rawText/.test(queue));
...
test('reporting that a file needs OCR does not change the regular-scan default', /regularPages: 3/.test(queue) && /mode: 'regular'/.test(queue));
test(
