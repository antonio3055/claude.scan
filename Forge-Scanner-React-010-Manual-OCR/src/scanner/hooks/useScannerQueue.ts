      ...
      try {
        if (validation.kind === 'pdf') {
          ...
          if (runOcr) {
            ...
            // A PDF with no usable text layer is a scan of paper. Reading it
            // needs OCR, which is reported here and run only when asked for.
            needsOcr = rawText.replace(/\s+/g, '').length < USABLE_TEXT_MIN_CHARS;
