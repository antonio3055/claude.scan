  ...
  return (
    <section className="scanner-panel extraction-panel">
      ...
      <div className="extraction-scroll">
        ...
        {/* Company and Contact carry the application's fields and nothing else. */}
        <div className="paired-info">
          <section><h3>Company</h3>
            <dl>
              <div><dt>Legal name</dt><dd>{value(app?.legalName)}</dd></div>
              <div><dt>DBA</dt><dd>{value(app?.dba)}</dd></div>
              <div><dt>EIN</dt><dd>{value(app?.ein)}</dd></div>
              <div><dt>DOB</dt><dd>{value(app?.dob)}</dd></div>
        ...
        {/*
          What the bank prints, kept apart from the application's own fields.
          Most applications carry no DBA and some statements do, and a bank
          often holds an older address, so this is worth reading — but it is
          the statements talking, not the company record.
        */}
        <section className="detail-section"><h3>On the bank statements</h3>
          <dl className="statement-identity">
            <div>
            ...
            <div>
              <dt>DBA{info.dbaDiffers ? <em className="differs" title="Different from the application"> differs</em> : null}</dt>
