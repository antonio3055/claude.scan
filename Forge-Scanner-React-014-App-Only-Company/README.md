# Forge Scanner React 014 — Application-Only Company Info

Standalone React + Vite scanner module, prepared for direct use inside the CRM and later Electron packaging.

Company info carries the application's fields and nothing else. What the bank statements say the company is — the name, any DBA, the address — is still read and still shown, in its own section further down, where it is clearly the statements talking and not the company record.

`Forge-Scanner-React-013-Clear-Labels` is untouched and remains available for rollback, as are 004 to 012.

| block | fields |
|---|---|
| **Company** | Legal name, DBA, EIN, DOB, Address — from the application |
| **Contact** | Owner, Mobile, Mobile 2, Mobile 3, Email — from the application |
| **On the bank statements** | Name, DBA, Address — from the statements, marked where they differ |

DBA (doing business as) and DOB (date of birth) are two different fields. Most
applications print no DBA; some bank statements do, which is why the statements
are read for one. 013 renamed the DBA row and moved the date of birth out of
Company; both are undone here — the labels were never the problem.

Nothing is extracted differently. The 60-document batch was re-run and compared
with 013 across 16 fields: 60 documents, 0 differences.
...
```bash
cat Forge-Scanner-React-014-App-Only-Company.zip.part* > Forge-Scanner-React-014-App-Only-Company.zip
unzip Forge-Scanner-React-014-App-Only-Company.zip
```
