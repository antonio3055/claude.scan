...
/**
 * Quote one argument for cmd.exe, the shell Windows spawn() runs a .cmd
 * shim through. Only every command/arg this project actually passes here
 * (vite, build, preview, --host, 127.0.0.1, --port, 8080, ...) needs this at
 * all -- none of it comes from outside input -- but it's still done
 * correctly rather than assumed safe.
 */
export function quoteForCmdExe(arg) {
...
function main(argv) {
  ...
  // npm installs local binaries like `vite` as a .cmd shim on Windows, which
  // Node's spawn() only resolves through a shell -- without this, every
  // command here fails with "spawn vite ENOENT" on Windows even though the
  // same command works fine everywhere else. Node deprecates (DEP0190)
  // shell:true combined with a separate args array, since the shell doesn't
  // escape them -- so on Windows the whole command line is built and quoted
  // here instead, and spawn gets a single string with no args array.
  const useShell = process.platform === "win32";
  const child = useShell
    ? spawn([command, ...args].map(quoteForCmdExe).join(" "), { stdio: "inherit", env, shell: true })
