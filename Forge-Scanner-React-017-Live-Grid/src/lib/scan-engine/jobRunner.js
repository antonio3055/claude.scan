...
import { MAX_JOB_ATTEMPTS, SCAN_TIMEOUTS, isTerminalErrorCode } from './scannerConfig.js';
 ...
 */
export function createCancellation(parent) {
  let cancelled = false;
  let aborts = new Set();
  let waiters = new Set();
  ...
  const token = {
    ...
    /** Detach a child token from its parent once its job is finished. */
    dispose() {
  ...
  // A child token is cancelled whenever its parent is, so Stop reaches the
  // stage running inside the current job without the job owning that wiring.
  if (parent) unlinkParent = parent.onCancel(() => token.cancel());
...
export async function runJobWithRetry(job, handlers) {
  const {
    ...
    jobLabel = (item) => item?.filename ?? 'File',
  ...
  while (attempt < maxAttempts) {
    attempt += 1;
    // Each attempt gets its own token. The whole-file watchdog cancels it, which
    // tears down whichever PDF or OCR stage is running inside the attempt, so a
    // file can never exceed its ceiling however it is stuck.
    const jobToken = createCancellation(parentToken);
    try {
      await withWatchdog(
        `${jobLabel(job)} (attempt ${attempt})`,
        jobTimeoutMs,
        () => processJob(job, attempt, jobToken, lane),
        { token: parentToken, abort: () => jobToken.cancel() }
      );
      return { outcome: 'complete', attempts: attempt };
    } catch (error) {
      if (resetWorkers) await runQuietly(() => resetWorkers(error, lane));
...
/**
 * Drain the queue. A job that fails, times out or throws unexpectedly is
 * recorded against that job only; the queue always continues to the next file.
 *
 * `concurrency` lanes read from the same queue at once. Each lane is the same
 * loop and carries its own lane number, which is how a job reaches the OCR
 * worker reserved for it — so one file's failure tears down only its own
 * worker. At one lane this is exactly the sequential queue, which is why there
 * is one implementation rather than a fast path beside a safe one.
 *
 * `getNext` must claim the job it returns, so two lanes never take the same
 * file.
 */
export async function runScanQueue(handlers) {
  const { getNext, shouldStop = () => false, beforeNext, concurrency = 1 } = handlers;
  ...
  const drain = async (lane) => {
    while (!shouldStop()) {
      ...
      await runJobWithRetry(job, { ...handlers, lane });
    }
  };

  await Promise.all(Array.from({ length: lanes }, (_unused, lane) => drain(lane)));
}
