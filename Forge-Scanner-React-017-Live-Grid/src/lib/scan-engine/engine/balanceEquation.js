...
(function (root) {
  ...
  /**
   * When nothing balances exactly, the signs a printed statement or a
   * keyword already fixed are kept, and only the genuinely unknown ones are
   * chosen — the assignment that comes closest to the printed ending wins.
   *
   * This never marks the reading verified. What it fixes is the reading
   * itself: without it, every component whose sign could not be proven fell
   * back to zero, so a real $75,266.74 in debits was reported as $0 rather
   * than as the best reading of what the bank actually printed — the
   * difference between an honestly unproven number and a wrong one.
   */
  function closestSigns(beginning, components, ending) {
    const choicesFor = (c) => (c.sign === null ? [1, -1, 0] : [c.sign]);
    const options = components.map(choicesFor);
    const combinations = options.reduce((total, list) => total * list.length, 1);
    if (!combinations || combinations > MAX_COMBINATIONS) {
      return components.map((c) => c.sign ?? 0);
    }
    ...
    for (let code = 0; code < combinations; code += 1) {
      ...
      const sum = components.reduce((acc, c, i) => acc + c.amount * signs[i], beginning);
      const gap = Math.abs(round2(sum) - ending);
      if (gap < bestGap) {
        bestGap = gap;
        best = signs;
      }
  ...
  function build(beginning, components, ending, layout) {
    ...
    const signs = solved ? solved.signs : closestSigns(beginning, components, ending);
