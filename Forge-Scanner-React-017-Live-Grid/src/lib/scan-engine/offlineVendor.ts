...
/**
 * Minimal local stand-ins for the types this file originally imported from
 * the full scanner app's `types/scanner.ts`. Only the fields this file
 * actually reads are declared -- validateFile only needs maxFileBytes, and
 * findDuplicate only needs the id/hash/size fields on whatever document
 * shape the caller uses.
 */
